...
{
    return [self URLForQuery:[NSString stringWithFormat:@"http://api.flickr.com/services/rest/?method=flickr.photos.search&license=1,2,4,7&has_geo=1&extras=original_format,tags,description,geo,date_upload,owner_name,place_url"]];
}
...
#define FLICKR_NEIGHBORHOOD_NAME @"place.neighbourhood._content"
#define FLICKR_NEIGHBORHOOD_PLACE_ID @"place.neighbourhood.place_id"
#define FLICKR_LOCALITY_NAME @"place.locality._content"
#define FLICKR_LOCALITY_PLACE_ID @"place.locality.place_id"
#define FLICKR_REGION_NAME @"place.region._content"
#define FLICKR_REGION_PLACE_ID @"place.region.place_id"
#define FLICKR_COUNTY_NAME @"place.county._content"
#define FLICKR_COUNTY_PLACE_ID @"place.county.place_id"
#define FLICKR_COUNTRY_NAME @"place.country._content"
#define FLICKR_COUNTRY_PLACE_ID @"place.country.place_id"

+ (NSString *)extractNameOfPlace:(id)placeId fromPlaceInformation:(NSDictionary *)place
{
    NSString *name = nil;
    
    if ([placeId isEqualToString:[place valueForKeyPath:FLICKR_NEIGHBORHOOD_PLACE_ID]]) {
        name = [place valueForKeyPath:FLICKR_NEIGHBORHOOD_NAME];
    } else if ([placeId isEqualToString:[place valueForKeyPath:FLICKR_LOCALITY_PLACE_ID]]) {
        name = [place valueForKeyPath:FLICKR_LOCALITY_NAME];
    } else if ([placeId isEqualToString:[place valueForKeyPath:FLICKR_COUNTY_PLACE_ID]]) {
        name = [place valueForKeyPath:FLICKR_COUNTY_NAME];
    } else if ([placeId isEqualToString:[place valueForKeyPath:FLICKR_REGION_PLACE_ID]]) {
        name = [place valueForKeyPath:FLICKR_REGION_NAME];
    } else if ([placeId isEqualToString:[place valueForKeyPath:FLICKR_COUNTRY_PLACE_ID]]) {
        name = [place valueForKeyPath:FLICKR_COUNTRY_NAME];
    }
